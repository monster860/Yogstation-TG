	...
	* ordered_locs(x = this.anchor_x, y = this.anchor_y, dir = this.anchor_dir, width = this.width, height = this.height, dwidth = this.dwidth, dheight = this.dheight) {
	...
	check_placement_validity(target, place_x, place_y, place_dir) {
		...
		for(let i = 0; i < ordered_source.length; i++) {
			...
			if(this.dmm.locate(sx, sy, 1, "/obj/effect/procedural_marker/turf_check_one")) {
				if(source_turf.istype(target_turf.type) || target_turf.istype(source_turf.type)) {
					turf_checks_one.set(source_area.type, true);
				} else if(!turf_checks_one.has(source_area.type)) {
					turf_checks_one.set(source_area.type, false);
				}
			}
